...
class CRUDBooster  {	
		...
		public static function getSetting($name, $group=''){	
			
			if(Cache::has('setting_'.$group.'_'.$name)) {
				return Cache::get('setting_'.$group.'_'.$name);
			}
			

		    $query = DB::table('cms_settings')->where('name',$name);

		    if($group != '')
		    {
		    	$query = $query->where('group_setting', $group);
		    }

		    $query = $query->first();
		    
		    Cache::forever('setting_'.$group.'_'.$name,$query->content);
